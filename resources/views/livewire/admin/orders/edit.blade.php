                ...
                <div>
                    ...
                    @if (isset($items['items']) && count($items['items']) > 0)
                        <x-tables.table>
                            ...
                            <x-slot name='tbody'>
                                @foreach ($items['items'] as $product_id => $item)
            ...
            <div class="bg-white shadow-md rounded p-4 py-2 ">
                ...
                {{-- TOTAL --}}
                    ...
                    <div class="grid grid-cols-2 justify-between gap-x-4  text-gray-600">
                        ...
                        <span class="text-right">$ {{number_format((isset($items['total']) )? $items['total']: 0 ,0,',','.')}}</span>
                        <label>Delivery </label>
                        <span class="text-right">$ {{ ($delivery_value) ? number_format( $delivery_value ,0,',','.') : 0 }}</span>
                        <label class="font-bold text-lg">Total </label>
                        <span class="text-right font-bold text-lg">$ {{number_format( ( isset($items['total']))? $items['total'] + (  ($delivery_value) ?  $delivery_value : 0 ) : 0 ,0,',','.')}}</span>
                    </div>
              
                
                {{-- BOTON --}}
                <x-jet-button class="w-full" wire:click="update">Actualizar</x-jet-button>    
