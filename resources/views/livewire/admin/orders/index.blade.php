<div>
    ...
    <div class='flex flex-wrap justify-between gap-4 pb-4'>
        <h1 class='text-2xl'>Pedidos <i wire:loading class='fas fa-spinner animate-spin text-2xl'></i> </h1>
                                    ...
                                    <x-tables.td>
                                        <div class='flex flex-wrap gap-x-2 justify-end'>
                                            ...
                                            @can('admin.orders.destroy')
                                                <div id='dropdown_delete_{{$order->id}}' x-data='{dropdown:false}' >
                                                    <div x-on:click='dropdown=true'>
                                                        <div>
                                                    ...
                                                    <div x-show='dropdown' x-cloak >
                                                        <x-modals.alert>
                                                            ...
                                                            <x-slot name='body'>
                                                                Seguro deseas eliminar {{$order->name}}?
                                                            </x-slot>
                                                            <x-slot name='footer'>
                                                                <x-jet-danger-button wire:click='delete({{$order->id}})'>
                                                                    Eliminar
                                                                </x-jet-danger-button>
                                ...
                                @else
                                    @if (isset($column['type']) && $column['type'] == 'date')
                                        <x-tables.td class='text-left'>
                                            ...
                                            {{ ($order->$name_column)? Helper::date($order->$name_column)->format('d-m-Y') : '' }}
                                        </x-tables.td>
                                        
                                    @elseif(isset($column['type']) && $column['type'] == 'money')
                                        <x-tables.td class='text-left'>
                                        $ {{ number_format($order->$name_column,0,',','.') }}
                                        </x-tables.td>
                                    ...
                                    @elseif(isset($column['type']) && $column['type'] == 'text')
                                        ...
                                        <div id='tooltip_{{$order->$name_column}}' x-data='{tooltip:false}' x-on:mouseleave='tooltip=false'>
                                            <div x-on:mouseover='tooltip=true' class='cursor-pointer'>
                                                {{ Str::limit($order->$name_column,10) }}
                                            </div>
