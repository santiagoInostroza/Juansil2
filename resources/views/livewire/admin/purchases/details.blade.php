<div>
    ...
    <div class='flex flex-wrap justify-between gap-4 pb-4'>
        <h1 class='text-2xl'>Detalle de compras <i wire:loading class='fas fa-spinner animate-spin text-2xl'></i> </h1>
       @can('admin.purchase_items.create')    
            <a href="{{ route('admin.purchase_items.create') }}" >
                <x-jet-button>
                    Crear Nombre
                </x-jet-button>
            </a>
                    ...
                    @foreach ($columns as $name_column => $column)
                        @if ($column['value'])
                                ...
                                @elseif ($name_column == 'purchase_id')
                                    <x-tables.td class='text-left'>
                                        @can('admin.purchases.show', $item)
                                            <a href="{{ route('admin.purchases.show', $item->purchase_id) }}">
                                                {{ $item->purchase_id }}
                                            </a>
                                        @else
                                        {{ $item->purchase_id }}
                                ...
                                @elseif ( $name_column == 'accions')
                                    <x-tables.td>
                                        <div class='flex flex-wrap gap-x-2 justify-end'>
                                            ...
                                            @can('admin.purchase_items.show')
                                                <a href="{{ route('admin.purchase_items.show', $item) }}">
                                                    <i class='fas fa-eye cursor-pointer px-2'></i>
                                                </a>
                                            @endcan
                                            @can('admin.purchase_items.edit')
                                                <a href="{{ route('admin.purchase_items.edit', $item) }}" >
                                                    <i class='fas fa-pen cursor-pointer px-2'></i>
                                                </a>
                                            @endcan
                                            @can('admin.purchase_items.destroy')
