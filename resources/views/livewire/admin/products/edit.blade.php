                ...
                <section class="grid gap-4 p-6 bg-white rounded-md shadow-md">
                    ...
                    <div
                        ...
                        @if ($image)
                            <label class="cursor-pointer">
                                <div class='py-1'>Cambiar imagen</div>
                                <img class="border-2 border-green-400 rounded h-48 w-full object-cover shadow-lg" src='{{ $image->temporaryUrl() }}'>
                                <x-jet-input wire:model="image" class='hidden' type="file"></x-jet-input>
                            </label>
                        @else
                            <label class='cursor-pointer'>
                                <div class='py-1'>Cambiar imagen</div>
                                <img class="border-2 border-green-400 rounded h-48 w-full object-cover shadow-lg" src="{{ Storage::url($product->image->url) }}">
                                <x-jet-input wire:model="image" class='hidden' type="file"></x-jet-input>
                            </label>
                        @endif
                        <!-- Progress Bar -->
                        ...
                        @error('image')<span class='text-red-500 text-sm p-1'>{{ $message }}</span>@enderror
                    </div>
                ...
                <section class="grid gap-4 | rounded-md shadow-md  p-6 bg-white ">
                    ...
                    <div>
                        <label>
                            ...
                            <textarea class='w-full h-24 p-2 border border-gray-300 rounded' placeholder='Descripción opcional' wire:model.defer='product.description'></textarea>
                        </label>
                        @error('product.description')<span class='p-1 text-sm text-red-500'>{{ $message }}</span>@enderror
                    </div>
                ...
                <section class="grid gap-4 | rounded-md shadow-md  p-6 bg-white">
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label>
                                ...
                                <x-selects.select :placeholder="'Selecciona un formato'" :collection='$formats' wire:model='product.product_format_id'/>
                            </label>
                            @error('product.product_format_id')<span class='p-1 text-sm text-red-500'>{{ $message }}</span>@enderror
                        </div>
                        ...
                        <div>
                            <label>
                                ...
                                <x-jet-input class='w-full p-2 border' placeholder='Ingresa cantidad' wire:model.defer='product.quantity_per_format'  >  </x-jet-input>
                            </label>
                            @error('product.quantity_per_format')<span class='p-1 text-sm text-red-500'>{{ $message }}</span>@enderror
                        </div>
                        ...
                        <div>
                            <label>
                                ...
                                <div class="flex items-center gap-2 mt-2">
                                    <span>$</span>
                                    <x-jet-input type="number" step="100" min="0" class='w-full p-2 border' wire:model.debounce.800ms='product.price' required></x-jet-input>
                                </div>
                            </label>
                            @error('product.price')<span class='p-1 text-sm text-red-500'>{{ $message }}</span>@enderror
                        </div>
                        ...
                        <div>
                            <label>
                                ...
                                <x-jet-input class='w-full p-2 border' type="number" min="0" wire:model.defer='product.stock_min'></x-jet-input>
                            </label>
                            @error('product.stock_min')<span class='p-1 text-sm text-red-500'>{{ $message }}</span>@enderror
                        </div>
                 ...
                 <section class="grid gap-4 p-6 bg-white rounded-md shadow-xl ">
                    ...
                    <label for=''>
                        ...
                        <x-selects.select :placeholder="'Selecciona marca'" :collection='$brands' wire:model='product.brand_id'/>
                        @error('product.brand_id')<span class='p-1 text-sm text-red-500'>{{ $message }}</span>@enderror
                    </label>
                </section>
            ...
            <article class="grid gap-4 ">
                ...
                <section class=" grid gap-x-4 | rounded-md shadow-xl   p-6 bg-white ">
                    <label class="flex justify-between items-center gap-x-2 | cursor-pointer ">
                        ...
                        <div class='switch'>
                            <input  type='checkbox' wire:model="product.is_active">
                ...
                <section x-data="{isOpenWholeSalePrice:@entangle('is_active_whole_sale_price') }" class=" grid gap-x-4 | rounded-md shadow-xl  p-6 bg-white ">
                    ...
                    <label class="flex justify-between items-center gap-x-2 | cursor-pointer ">
                        ...
                        <div class="switch">
                            <input  type="checkbox" wire:model="product.is_active_whole_sale_price">
                            <span class="slider round"></span> 
                        </div>
                    </label>
                    <p class="mt-2 text-sm text-gray-400">
                    ...
                    <label x-show="isOpenWholeSalePrice" x-cloak class="mt-4">
                        ...
                        <div class="flex items-center gap-2">
                           $ <x-jet-input class='w-full p-2 border' type="number" step="10" min="0" wire:model.defer='product.whole_sale_price'></x-jet-input>
                        </div>
                        @error('product.whole_sale_price')<span class='p-1 text-sm text-red-500'>{{ $message }}</span>@enderror
                    </label>
                ...
                <section class=" grid gap-x-4 | rounded-md shadow-xl  p-6 bg-white ">
                    ...
                    <label class="mt-4">
                        <div class="flex items-center gap-2">
                           <x-jet-input class='w-full p-2 border' type="number" step="1" min="0" wire:model.defer='product.expiration_notice_days'></x-jet-input>
                        </div>
                        @error('product.expiration_notice_days')<span class='p-1 text-sm text-red-500'>{{ $message }}</span>@enderror
                    </label>
                ...
                <section x-data="{isOpenOffer:@entangle('has_offer'), isOpenFixedOffer:@entangle('fixed_offer') }"  class="grid gap-4 | rounded-md shadow-md  p-6 bg-white">
                    ...
                    <label class="flex items-center justify-between">
                        ...
                        <div class='switch'>
                            <input  type='checkbox' wire:model="product.has_offer">
                            <span class='slider round'></span> 
                        </div>
                    </label>
                    ...
                    <div x-show="isOpenOffer" x-cloak x-transition class="flex flex-col gap-4">
                        ...
                        <div>
                            <label> 
                                ...
                                <div class="flex items-center gap-2">
                                    <span>$</span>
                                    <x-jet-input class='w-full p-2 border' type="number" min="0" wire:model.debounce.800='product.offer_price'></x-jet-input>
                                </div>
                            </label>
                            @error('product.offer_price')<span class='p-1 text-sm text-red-500'>{{ $message }}</span>@enderror
                        </div>
                        ...
                        <div>
                            <label> 
                                ...
                                <div class="flex items-center gap-2">
                                    <span>%</span>
                                    <x-jet-input class='w-full border' type="number" min="0" step="0.01" wire:model.debounce.800='product.discount_rate'></x-jet-input>
                                </div>
                            </label>
                            @error('product.discount_rate')<span class='p-1 text-sm text-red-500'>{{ $message }}</span>@enderror
                        </div>
                        ...
                        <div>
                            <label class="flex items-center justify-between col-span-2 gap-4">
                                ...
                                <div class='switch'>
                                    <input  type='checkbox' wire:model="product.fixed_offer">
                                    <span class='slider round'></span> 
                                </div>
                            </label>
                        ...
                        <div x-cloak x-show="!isOpenFixedOffer">
                            ...
                            <div>
                                <label class="">
                                    ...
                                    <x-jet-input class='w-full p-2 border' type="date" wire:model.defer='product.offer_start_date'></x-jet-input>
                                </label>
                            ...
                            <div class="mt-3">
                                <label class="">
                                    ...
                                    <x-jet-input class='w-full p-2 border' type="date" wire:model.defer='product.offer_end_date'></x-jet-input>
                                </label>
                                @error('product.offer_end_date')<span class='p-1 text-sm text-red-500'>{{ $message }}</span>@enderror
                            </div>
